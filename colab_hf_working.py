#!/usr/bin/env python3
"""
Colab 脚本: 工作版本 - 从 HuggingFace Datasets 下载 K 线数据
...
实际列名: open_time, open, high, low, close, volume, close_time
"""
...
class HuggingFaceKlinesDownloader:
    ...
    def download_klines_from_hf(self, trading_pair, timeframe="15m"):
        ...
        Returns:
            DataFrame: K 线数据 (列名: open_time, open, high, low, close, volume, close_time)
        """
        try:
    ...
    def fetch_and_combine(self, trading_pair, timeframe="15m", rows=10000):
        ...
        if df is not None:
            ...
            print(f"  原始列名: {actual_columns}")
            
            # 列名映射 (HuggingFace 的列名 -> 标准列名)
            column_mapping = {
                'open_time': 'timestamp',
                'open': 'open',
                'high': 'high',
                ...
                'close': 'close',
                'volume': 'volume',
                'close_time': 'close_time'  # 保留但后续不使用
            }
            ...
            if all(col in df.columns for col in required_columns):
                ...
                print(f"  ✓ 列名已标准化: {df.columns.tolist()}")
            else:
                print(f"  ✗ 列名处理失败")
                return None
...
def main():
    ...
    print("="*60)
    print("HuggingFace K 线数据下载工具 (工作版)")
    print("="*60)
    ...
    if df_btc is not None:
        ...
        print(f"  列名: {df_btc.columns.tolist()}")
